...
class Reflection {
    ...
    //modify by cao_yj 
    //String troyPath = System.getProperty("user.dir");
    //String[] troyLib = troyPath.split("/");
    //StringBuffer sb = new StringBuffer();
    //for (int i = 0;i<troyLib.length -2;i++ ){
    //    sb.append(troyLib[i] + "/");
    //} 
    //troyPath = sb.append("lib").toString();

    //File[] troyFile = new File(troyPath).listFiles();
    //for (int i = 0;i<troyFile.length ;i++ ){
    //    if(!troyFile[i].exists())
    //        continue;

    //    if(troyFile[i].toString().endsWith(".jar") || troyFile[i].toString().endsWith(".zip")){
    //        htClasspath.put(troyFile[i].toString(),"");
    //    }else{
    //        if(troyFile[i].isDirectory()){
    //            htClasspath.put(troyFile[i].toString(),"");
    //        }
    //    } 
    //}
