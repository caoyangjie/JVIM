...
let s:magenta		= exists("g:magenta") ? g:magenta :	    '#7F0055'
...
"设置鼠标所在位置行样式
exec 'hi CursorColumn ctermfg=7 ctermbg=7 cterm=bold guifg=' . s:boldWhite . ' guibg=fg'
exec 'hi CursorLine   cterm=underline gui=underline guibg=bg'
exec 'hi Cursor       cterm=reverse guifg=bg guibg=fg'
exec 'hi lCursor      cterm=reverse guifg=bg guibg=fg'
"设置折叠行的样式
exec 'hi Folded       ctermfg=3 ctermbg=7 guifg=' . s:brown . ' guibg=' . s:boldBlack
exec 'hi FoldColumn   ctermfg=3 ctermbg=7 guifg=' . s:brown . ' guibg=' . s:boldBlack
"设置注释行样式
exec "hi comment      gui=none	guifg=".s:boldCyan	"guibg=".s:none
"exe 'hi Comment      guifg=' . s:boldCyan
"设置搜索样式
exec 'hi IncSearch    cterm=reverse gui=reverse'
exec 'hi Search       guifg=bg guibg=' . s:brown

exec 'hi Pmenu        guibg=' . s:magenta
exec 'hi PmenuSel     guibg=bg'
exec 'hi PmenuSbar    ctermfg=3 guifg=' . s:brown . ' guibg=fg'
exec 'hi PmenuThumb   cterm=reverse gui=reverse'
...
exec "hi statement    gui=none	guifg=".s:boldMagenta 		"guibg=".s:none
exec "hi todo	      gui=none	guifg=".s:black		"guibg=".s:yellow
"设置类型样式
exec "hi type	      gui=none	guifg=".s:boldMagenta		"guibg=".s:none
